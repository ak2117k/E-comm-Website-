  ...
  return async function getProductsThunk(dispatch) {
    ...
    try {
      ...
      if (result.length === 0) {
        ...
        response = await fetch("https://koovs-api-data.onrender.com/mens");
        ...
        console.log(result);
        allProducts = await result.filter((item) => {
          return (
            category.includes(item.brand) ||
            category.includes(item.description) ||
            category.includes(item.info) ||
      ...
      console.log("*******");
      const updatedProducts = await updateProductDetails(result); // Update product details
